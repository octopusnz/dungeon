...
pub enum MainAction { PickPocket, Inventory, Shop, Fight, Tavern, Exit }

pub fn prompt_main_action() -> MainAction {
    use std::io::{self, Write};
    println!("\n===== Actions =====");
    println!("[P]ickpocket  [I]nventory  [S]hop  [F]ight  [T]avern  E[x]it / [Q]uit");
    print!("Enter choice: "); let _ = io::stdout().flush();
    let mut line = String::new(); if io::stdin().read_line(&mut line).is_err() { return MainAction::Exit; }
    let input = line.trim(); if input.is_empty() { return MainAction::PickPocket; }
    match input.chars().next().unwrap().to_ascii_lowercase() {
        ...
        other => { println!("Unrecognized option '{}'. (P/I/S/F/T/Q)", other); MainAction::PickPocket }
    }
}

pub fn print_simple_header(title: &str) { println!("\n──── {} ────", title); }

pub fn print_event_summary(title: &str, before: &Inventory, after: &Inventory, items_added: &[String], items_removed: &[String]) {
...
fn format_delta(delta: i64, unit: &str) -> String { if delta > 0 { format!("+{} {}", delta, unit) } else { format!("{} {}", delta, unit) } }
fn summarize_items(items: &[String]) -> String { if items.len() <= 5 { items.join(", ") } else { format!("{} items", items.len()) } }